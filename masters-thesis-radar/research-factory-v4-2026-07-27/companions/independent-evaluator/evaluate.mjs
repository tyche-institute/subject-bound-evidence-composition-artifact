...
const packetPath = path.resolve(
  here,
  "../../masters-thesis-radar/research-factory-v4-2026-07-27/external-label-packet/transactions-for-labelling.json",
);
const outputPath = path.join(here, "independent-results-unsealed.json");
const layerPath = path.join(here, "independent-layer-results-unsealed.json");
...
if (evaluations.length !== 90) {
  throw new Error(`expected 90 transactions, got ${evaluations.length}`);
}
if (new Set(evaluations.map((item) => item.transaction_id)).size !== 90) {
  throw new Error("transaction IDs are not unique");
}

const answers = evaluations.map((item) => ({
  transaction_id: item.transaction_id,
  labeller_id: "independent-js-v1",
