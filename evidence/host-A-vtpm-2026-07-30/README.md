# host-A identified-VM and Microsoft-vTPM evidence

This directory preserves an execution on the configured `host-A` SSH target,
identified by hashed host state and the adjacent SSH host-key fingerprints in
`HOST-A-RUN-OVERLAY.json`. It is a distinct Ubuntu 24.04 x86-64 virtual-machine
...
preflight checksum file retains its original host-A-relative source paths;
